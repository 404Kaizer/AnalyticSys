...
function renderDgVisaoGeralPdf(results, thresholds, dtIni, dtFim) {
  ...
  // Cache pro Relatório Gerencial (gerarRelatorioGerencialDG, mais abaixo)
  // — guarda só os números já calculados aqui (nada de recomputar, nada de
  // raspar texto da tela). Gráficos/donuts o relatório lê direto do DOM
  // (canvas → imagem, SVG → clonado) no momento em que o botão é clicado.
  window._dgVgReportData = {
    periodoLabel: (document.getElementById('dg-cal-label')?.textContent || 'Período não selecionado').trim(),
    varTotalFisica, custoTotal, estTotais, movTotais, custoMovTotais,
    totalEstTeoricoKpi, veiculosTotalKpi,
    fechExcluidosCount: sapFechExcluidosPeriodo.length,
    counts, scoreInfo,
    extRegional, extCentral,
    nCentrais: results.length
  };
}
...
// ────────────────────────────────────────────
// RELATÓRIO GERENCIAL — usa o MESMO shell dos outros relatórios do
// sistema (_buildRankingShellHTML/_openRelWindow, em relatorio.js) em vez
// de reinventar um layout de impressão — é esse o motivo da primeira
// versão ter saído feia/quebrada: tentava reaproveitar o CSS do app (feito
// pra um shell de SPA com sidebar) fora do contexto dele. Aqui a lógica é
// a mesma dos outros relatórios: monta um bodyHtml autocontido (CSS
// próprio, embutido) e entrega pro shell compor cabeçalho/rodapé/botão de
// impressão.
//
// Números vêm do cache (window._dgVgReportData, gravado ao final de
// renderDgVisaoGeralPdf) — nada recalculado aqui. Gráficos (canvas) e
// donuts (SVG) são lidos direto do DOM no momento do clique: canvas vira
// imagem (não sobrevive a clonagem), SVG é só clonado (sobrevive normal).
// ────────────────────────────────────────────

function _dgrColorFor(v) {
  return v < -0.0001 ? '#f87171' : v > 0.0001 ? '#fbbf24' : '#94a3b8';
}

// Snapshot de um <canvas> (Chart.js) como <img> em base64 — string vazia
// se o canvas não existir ou estiver vazio (sem quebrar o relatório).
function _dgrSnapshotCanvas(id) {
  const el = document.getElementById(id);
  if (!el || el.tagName !== 'CANVAS') return '';
  try { return el.toDataURL('image/png', 1.0); } catch (e) { return ''; }
}

// SVG clona normal via outerHTML (diferente de canvas, não perde o
// desenho) — string vazia se o elemento não existir.
function _dgrCloneSvg(id) {
  const el = document.getElementById(id);
  return el ? el.outerHTML : '';
}

function _dgrDonutBox(titulo, svgId) {
  const svg = _dgrCloneSvg(svgId);
  return `<div class="dgr-donut-box">
    <div class="dgr-donut-title">${_rankEsc(titulo)}</div>
    ${svg || '<div class="donut-empty">Sem dados</div>'}
  </div>`;
}

function _dgrFeaturedCard(label, iconCls, valorStr, unidade, col, pct, pctLabel, extraHtml) {
  const barWidth = pct === null || pct === undefined ? 0 : Math.min(100, Math.abs(pct));
  const pctStr = pct === null || pct === undefined ? '—' : `${Math.abs(pct).toLocaleString('pt-BR',{minimumFractionDigits:2,maximumFractionDigits:2})}%`;
  return `
    <div class="dgr-featured-card" style="border-top-color:${col}">
      <div class="dgr-featured-label"><i class="ti ${iconCls}"></i> ${_rankEsc(label)}</div>
      <div class="dgr-featured-value" style="color:${col}">${_rankEsc(valorStr)}</div>
      <div class="dgr-featured-unit">${_rankEsc(unidade)}</div>
      ${extraHtml || ''}
      <div class="dgr-featured-pct" style="color:${col}">${pctStr}<span class="dgr-featured-pct-label">${_rankEsc(pctLabel)}</span></div>
      <div class="dgr-pct-bar"><div class="dgr-pct-bar-fill" style="width:${barWidth}%;background:${col}"></div></div>
    </div>`;
}

function _dgrMiniCard(label, iconCls, valorStr, subStr, col) {
  return `<div class="dgr-mini-card">
    <div class="dgr-mini-label"><i class="ti ${iconCls}"></i> ${_rankEsc(label)}</div>
    <div class="dgr-mini-value"${col ? ` style="color:${col}"` : ''}>${_rankEsc(valorStr)}</div>
    ${subStr ? `<div class="dgr-mini-sub">${_rankEsc(subStr)}</div>` : ''}
  </div>`;
}

function _dgrExtremoCard(titulo, item, unidade) {
  if (!item) return `<div class="dgr-mini-card"><div class="dgr-mini-label">${_rankEsc(titulo)}</div><div class="dgr-mini-sub">Sem dados</div></div>`;
  const col = _dgrColorFor(item.v);
  return `<div class="dgr-mini-card">
    <div class="dgr-mini-label">${_rankEsc(titulo)}</div>
    <div class="dgr-mini-value" style="color:${col}">${money(Math.abs(item.v))}</div>
    <div class="dgr-mini-sub">${fmtKg(Math.abs(item.kg || 0))} ${_rankEsc(unidade)} · <strong style="color:#e2e8f0">${_rankEsc(item.k)}</strong></div>
  </div>`;
}

// Tabelas do Detalhado Analítico e de Giro & Cobertura já estão prontas no
// DOM (innerHTML) — clona o conteúdo em vez de reconstruir do zero. O CSS
// pra essas classes (.da-table, .dg-giro-mat-row etc.) é escrito de novo
// aqui dentro do relatório (autocontido, tema escuro equivalente ao do
// dashboard) — não depende de nenhum arquivo CSS do app.
function _dgrCloneTable(containerId) {
  const el = document.getElementById(containerId);
  return el ? el.innerHTML : '<div class="donut-empty">Sem dados</div>';
}

function gerarRelatorioGerencialDG() {
  const d = window._dgVgReportData;
  if (!d) { alert('Clique em "Atualizar" e aguarde os dados da Visão Geral carregarem antes de gerar o relatório.'); return; }
  if (!d.nCentrais) { alert('Sem dados no período selecionado.'); return; }

  const now = new Date().toLocaleString('pt-BR');
  const varCol = _dgrColorFor(d.varTotalFisica);
  const cstCol = _dgrColorFor(d.custoTotal);
  const evoKg  = d.estTotais.totalFim - d.estTotais.totalIni;
  const evoCustoVal = (d.estTotais.custoFim || 0) - (d.estTotais.custoIni || 0);
  const evoCol = _dgrColorFor(evoKg);
  const evoPct = Math.abs(d.estTotais.totalIni) > 0.0001 ? (evoKg / d.estTotais.totalIni) * 100 : null;

  const custoEstTeoricoKpi = (d.estTotais.custoIni||0) + (d.custoMovTotais.custoEnt||0) - (d.custoMovTotais.custoSai||0);
  const pctCustoKpi = Math.abs(custoEstTeoricoKpi) > 0.0001 ? (d.custoTotal / custoEstTeoricoKpi) * 100 : null;
  const pctVarKpi   = Math.abs(d.totalEstTeoricoKpi) > 0.0001 ? (d.varTotalFisica / d.totalEstTeoricoKpi) * 100 : null;

  const veiculosExtra = `
    <div class="dgr-veiculos-row">
      <span style="color:${_dgrColorFor(d.veiculosTotalKpi.caminhoes)}"><i class="ti ti-truck"></i>${_daFmtCountSigned(d.veiculosTotalKpi.caminhoes)}</span>
      <span style="color:${_dgrColorFor(d.veiculosTotalKpi.carretas)}"><i class="ti ti-container"></i>${_daFmtCountSigned(d.veiculosTotalKpi.carretas)}</span>
      <span style="color:${_dgrColorFor(d.veiculosTotalKpi.ibcs)}"><i class="ti ti-box"></i>${_daFmtCountSigned(d.veiculosTotalKpi.ibcs)}</span>
    </div>`;

  const custoAbsHtml = DG_VG_CAT_ORDER.map(catKey =>
    _dgrDonutBox(DG_VG_CAT_LABELS[catKey] || catKey, `dg-vg-donut-${catKey}`)
  ).join('');
...
${d.fechExcluidosCount ? `<div class="dgr-badge-fech"><i class="ti ti-calendar-check"></i> ${d.fechExcluidosCount} ajuste(s) de fechamento desconsiderado(s) no período</div>` : ''}
...
<div class="dgr-section">
  ...
  <div class="dgr-featured-row">
    ${_dgrFeaturedCard('Variação', 'ti-scale', `${varSymbol(d.varTotalFisica)} ${fmtKg(Math.abs(d.varTotalFisica))}`, 'kg bruto', varCol, pctVarKpi, 'do Est. Teórico', veiculosExtra)}
    ${_dgrFeaturedCard('Custo Var.', 'ti-currency-dollar', `${varSymbol(d.custoTotal)} ${money(Math.abs(d.custoTotal))}`, 'R$ bruto', cstCol, pctCustoKpi, 'do Custo Teórico')}
  </div>
  <div class="dgr-mini-grid">
    ${_dgrMiniCard('Est. Inicial Total', 'ti-archive', money(d.estTotais.custoIni||0), fmtKg(d.estTotais.totalIni))}
    ${_dgrMiniCard('Entradas', 'ti-arrow-bar-to-down', money(d.custoMovTotais.custoEnt||0), fmtKg(d.movTotais.totalEnt))}
    ${_dgrMiniCard('Saídas', 'ti-arrow-bar-up', money(d.custoMovTotais.custoSai||0), fmtKg(d.movTotais.totalSai))}
    ${_dgrMiniCard('Est. Final Total', 'ti-box', money(d.estTotais.custoFim||0), fmtKg(d.estTotais.totalFim))}
    ${_dgrMiniCard('Evolução Estoque', 'ti-trending-up', (evoPct===null?'—':(varSymbol(evoKg)+' '+Math.abs(evoPct).toLocaleString('pt-BR',{maximumFractionDigits:1})+'%')), `${money(Math.abs(evoCustoVal))} · ${fmtKg(Math.abs(evoKg))}`, evoCol)}
  </div>
</div>
...
<div class="dgr-section">
  ...
  <div class="dgr-donut-grid-2">
    ...
    ${_dgrDonutBox('Saúde Geral — Centrais', 'dg-vg-gauge-central-svg')}
  </div>
</div>
...
<div class="dgr-section">
  ...
  <div class="dgr-extremos-grid">
    ...
    ${_dgrExtremoCard('Regional · Maior Sobra',     d.extRegional.max && d.extRegional.max.v > 0 ? d.extRegional.max : null, 'variação')}
    ${_dgrExtremoCard('Central · Maior Desfalque',  d.extCentral.min  && d.extCentral.min.v  < 0 ? d.extCentral.min  : null, 'variação')}
    ${_dgrExtremoCard('Central · Maior Sobra',      d.extCentral.max  && d.extCentral.max.v  > 0 ? d.extCentral.max  : null, 'variação')}
  </div>
  <div class="dgr-chart-box"><img src="${_dgrSnapshotCanvas('dg-vg-chart-regional')}" alt=""></div>
...
<div class="dgr-section">
  ...
  <div class="dg-giro-dual-grid">
    ...
    <div class="dg-giro-mat-card"><div class="dg-giro-mat-header"><span class="dg-top5-title" style="color:#f87171"><i class="ti ti-trending-down"></i> Top 5 Centrais — Mais Críticas</span></div><div class="dg-giro-mat-body">${_dgrCloneTable('dg-vg-giro-central-baixo-body')}</div></div>
  </div>
  <div class="dg-giro-dual-grid">
    <div class="dg-giro-mat-card"><div class="dg-giro-mat-header"><span class="dg-top5-title" style="color:#34d399"><i class="ti ti-trending-up"></i> Top 5 Materiais — Mais Saudáveis</span></div><div class="dg-giro-mat-body">${_dgrCloneTable('dg-vg-giro-mat-alto-body')}</div></div>
    <div class="dg-giro-mat-card"><div class="dg-giro-mat-header"><span class="dg-top5-title" style="color:#f87171"><i class="ti ti-trending-down"></i> Top 5 Materiais — Mais Críticos</span></div><div class="dg-giro-mat-body">${_dgrCloneTable('dg-vg-giro-mat-baixo-body')}</div></div>
  </div>
</div>
...
<div class="dgr-section">
  ...
  const html = _buildRankingShellHTML({
    ...
    subtitle:   'Estoque, movimentação, variação e custo consolidados do período — visão executiva completa.',
    bodyHtml,
    notaRodape: 'Valores calculados a partir dos lançamentos e movimentações SAP do período selecionado. Ajustes de Fechamento Mensal são desconsiderados do cálculo de variação.'
  });
